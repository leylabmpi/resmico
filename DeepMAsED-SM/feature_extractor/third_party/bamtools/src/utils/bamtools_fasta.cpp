...
struct Fasta::FastaPrivate
{
    ...
    /**
     * Get entire sequence by name
     * @param name the name of the sequence (whatever comes after > in the fasta file)
                                      ...
                                      std::string& sequence)
{
    ...
    if (HasIndex && !Index.empty()) {
        ...
        // validate reference id
        if (idx == Index.size()) {
            std::cerr << "FASTA error: invalid name specified: " << name << std::endl;
            return false;
        }

        // retrieve reference index data
        const FastaIndexData& referenceData = Index.at(idx);
