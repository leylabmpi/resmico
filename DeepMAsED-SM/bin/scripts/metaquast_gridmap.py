...
# debug
def _run_quast_per_ref_DEBUG(quast_py_args, output_dirpath_per_ref, ref_fpath,
                       ref_assemblies, total_num_notifications, sys_path, is_parallel_run):
    sys.path = sys_path
    import quast
    return sys.path

def _run_quast_per_ref(quast_py_args, output_dirpath_per_ref, ref_fpath, ref_assemblies,
                       total_num_notifications, sys_path, is_parallel_run):
    sys.path = sys_path
...
def main(args):
    ...
    if not qconfig.memory_efficient:
        ...
        threads_per_ref = qconfig.max_threads
        ...
        for ref_fpath, ref_assemblies in assemblies_by_reference:
            ref_name, json_text, total_num_notifications = \
                _run_quast_per_ref(quast_py_args, output_dirpath_per_ref,
                                   ref_fpath, ref_assemblies, total_num_notifications, False)
            if not ref_name:
                continue
