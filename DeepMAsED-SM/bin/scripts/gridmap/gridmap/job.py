...
class JobMonitor(object):
    ...
    def __init__(self, temp_dir=DEFAULT_TEMP_DIR):
        ...
        self.id_to_job = {}
        self.vmem_increase_gigabytes = 2
...
def _send_mail(subject, body_text, attachments=None):
    ...
    try:
        ...
        logger.error('Unknown error when trying to connect to SMTP server',
                     exc_info=True)
        return 
...
def handle_resubmit(session_id, job, temp_dir=DEFAULT_TEMP_DIR):
    ...
    if job.num_resubmits < NUM_RESUBMITS:
        ...
        job.num_resubmits += 1
        job.cause_of_death = ""
        ## doubling job resources
        job.h_vmem = '{}G'.format(int(job.h_vmem.rstrip('G')) * 2)
        logger.info('h_vmem increased to {}'.format(job.h_vmem))
        h_rt = job.h_rt.split(':')
        h_rt = int(h_rt[0]) * 360 + int(h_rt[1]) * 60 + int(h_rt[2])
        h_rt = h_rt * 2
        h_rt = 60 if h_rt < 60 else h_rt
        hours = int(h_rt / 360.0)
        minutes = int((h_rt - hours * 360) / 60.0)
        seconds = int(h_rt - hours * 360 - minutes * 60)
        job.h_rt = '{0:02d}:{1:02d}:{2:02d}'.format(hours, minutes, seconds)
        logger.info('h_rt increased to {}'.format(job.h_rt))
        ## resubmitting
...
def process_jobs(jobs, temp_dir=DEFAULT_TEMP_DIR, white_list=None, quiet=True,
    ...
    if (not local and not DRMAA_PRESENT):
        logger = logging.getLogger(__name__)
        if require_cluster:
            msg = 'Could not import drmaa, but cluster access required.'
            raise DRMAANotPresentException(msg)
