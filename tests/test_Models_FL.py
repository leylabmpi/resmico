...
class TestBinaryDataEval(unittest.TestCase):
    ...
    def test_batching_multiple_per_batch(self):
        reader = ContigReader.ContigReader('./data/preprocess/', Reader.feature_names, 1, False, False)
    ...
    def test_gen_eval_data(self):
        for cached in [False, True]:
            reader = ContigReader.ContigReader('./data/preprocess/', Reader.feature_names, 1, False, cached)
            indices = np.arange(len(reader))
    ...
    def test_gen_eval_data_short_window(self):
        reader = ContigReader.ContigReader('./data/preprocess/', Reader.feature_names, 1, False)
        indices = np.arange(len(reader))
        eval_data = Models_FL.BinaryDataEval(reader, indices, Reader.feature_names, 50, 30, 1e5, False)
        self.assertEqual(1, len(eval_data))
        self.assertEqual(2, len(eval_data.batch_list[0]))
    ...
    def test_gen_eval_data_cached(self):
        reader = ContigReader.ContigReader('./data/preprocess/', Reader.feature_names, 1, False)
        indices = np.arange(len(reader))
        eval_data = Models_FL.BinaryDataEval(reader, indices, Reader.feature_names, 500, 250, 1e5, True)
        self.assertEqual(1, len(eval_data))
        self.assertEqual(2, len(eval_data.batch_list[0]))
        self.assertTrue(all(a == b for a, b in zip(eval_data[0][0][0][0:6], [1, 0, 0, 0, 2, 1])))
        self.assertTrue(all(a == b for a, b in zip(eval_data[0][0][5][0:6], [1, 0, 0, 0, 0, 0])))
    ...
    def test_gen_eval_data_cache_in_contig_reader(self):
        reader = ContigReader.ContigReader('./data/preprocess/', Reader.feature_names, 1, False, True)
        indices = np.arange(len(reader))
        eval_data = Models_FL.BinaryDataEval(reader, indices, Reader.feature_names, 500, 250, 1e5, False)
        self.assertEqual(1, len(eval_data))
        self.assertEqual(2, len(eval_data.batch_list[0]))
    ...
    def test_gen_eval_data_cache_in_contig_reader_chunked(self):
        reader = ContigReader.ContigReader('./data/preprocess/', Reader.feature_names, 1, True, True)
        indices = np.arange(len(reader))
        eval_data = Models_FL.BinaryDataEval(reader, indices, Reader.feature_names, 500, 250, 1e5, False)
        self.assertEqual(1, len(eval_data))
        self.assertEqual(2, len(eval_data.batch_list[0]))
