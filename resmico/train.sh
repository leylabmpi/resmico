...
N9K_D20="/cluster/work/grlab/projects/projects2019-contig_quality/data/v2/resmico-sm/GTDBr202_n9k_train_d20m/features"
...
declare -a DATA_DIRS=("${N9K}" "${N9K_D12}" "${N9K_D20}" "${N9K_CAMI}" "${N9K_GB07}")
...
do
  if [ ! -d "${DATA_DIR}" ]; then
    echo "Directory ${DATA_DIR} does not exist. Bailing out."
    exit 1
  fi
  ...
  cmd3="/usr/bin/time python resmico train --binary-data --feature-files-path ${SCRATCH_DIR} \
      ...
      --max-len ${max_len} --gpu-eval-mem-gb 1 --features ${features_small} --n-epochs 30 \
      --num-translations ${num_translations} --max-translation-bases ${max_translation_bases} \
      --min-avg-coverage 0 --mask-padding --cache-train --net-type cnn_resnet_avg \
